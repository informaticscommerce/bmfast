...
namespace Mage4\ExtendAmastyRequestQuote\Model\ResourceModel;
...
class Quote extends \Amasty\RequestQuote\Model\ResourceModel\Quote
{
     ...
     * @return \Amasty\RequestQuote\Model\ResourceModel\Quote
     */
    ...
    public function loadMagentoQuoteByIdWithoutStore($quote, $quoteId)
    {
        $connection = $this->getConnection();
        if ($connection) {
            $select = \Magento\Quote\Model\ResourceModel\Quote::_getLoadSelect('entity_id', $quoteId, $quote);
    ...
    {
        $select = \Magento\Quote\Model\ResourceModel\Quote::_getLoadSelect($field, $value, $object);
